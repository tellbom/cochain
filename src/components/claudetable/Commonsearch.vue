<template>
    <div class="common-search">
        <el-form ref="formRef" :model="searchForm" :inline="true" class="search-form" @submit.prevent="handleSearch">
            <!-- 动态渲染搜索字段 -->
            <template v-for="field in visibleFields" :key="field.prop">
                <el-form-item :label="field.label" :prop="field.prop">
                    ...
                    <!-- 自定义插槽 -->
                    <slot v-else-if="field.type === 'slot'" :name="field.prop" :field="field" :form="searchForm"></slot>
                </el-form-item>
            </template>
...
.common-search {
    ...
    padding: var(--co-space-20) var(--co-space-24);
    margin-bottom: var(--co-space-16);
...
.expand-btn {
    margin-left: var(--co-space-4);
    padding: var(--co-space-8) var(--co-space-12);
...
:deep(.el-input__wrapper) {
    ...
    transition: box-shadow var(--co-transition-fast);
}

:deep(.el-input__wrapper:hover) {
    box-shadow: 0 0 0 1px var(--co-ink-disabled) inset;
}

:deep(.el-input__wrapper.is-focus) {
    box-shadow: 0 0 0 1.5px var(--co-primary) inset;
}
...
:deep(.el-button) {
    ...
    transition:
        ...
        border-color var(--co-transition-fast),
