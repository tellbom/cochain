<template>
    <div class="common-table">
        ...
        <div class="table-toolbar">
            ...
            <div class="toolbar-right">
                ...
                <el-button type="primary" :icon="Setting" circle @click="columnSettingVisible = true" title="字段设置" />
            </div>
        </div>
    ...
    -->
        <el-table ref="tableRef" :data="tableData" style="width: 100%" :header-cell-style="headerCellStyle" @selection-change="handleSelectionChange">
            <!-- 多选列 -->
            <el-table-column v-if="showSelection" type="selection" width="55" align="center" fixed="left" />

            <!-- 单选列 -->
            <el-table-column v-if="showRadio" label="选择" width="55" align="center" fixed="left">
                <template #default="scope">
                    <el-radio v-model="radioSelected" :label="scope.row[rowKey]" @change="handleRadioChange(scope.row)"> &nbsp; </el-radio>
                </template>
            </el-table-column>
            ...
            <!-- 操作列 -->
            <el-table-column v-if="showOperation" label="操作" fixed="right" :width="operationWidth">
                <template #default="scope">
        ...
        <!-- 字段显示设置抽屉 -->
        <el-drawer v-model="columnSettingVisible" title="字段显示设置" direction="rtl" size="360px">
...
const emit = defineEmits(['selection-change', 'radio-change', 'page-change', 'size-change', 'update:currentPage', 'update:pageSize'])
...
const headerCellStyle = {
    ...
    fontWeight: '600',
    fontSize: '13px',
    borderBottom: '1px solid var(--co-divider)',
}
...
const initColumns = () => {
    ...
    if (savedColumns) {
        try {
            ...
            selectedColumns.value = selectedColumns.value.filter((col) => allColumns.value.some((c) => c.prop === col))
        } catch {
...
/* ================================================================
   ...
   改造说明：
     ...
     ✓ 所有颜色替换为 --co-* Token
     ✓ 圆角统一为 --co-radius-* Token
     ✓ 行 hover 使用主色浅背景 --co-primary-light
   ================================================================ */
...
:deep(.toolbar-right .el-button.is-circle) {
    ...
    color: var(--co-ink-2);
    transition:
        background var(--co-transition-fast),
        color var(--co-transition-fast),
        transform var(--co-transition-fast);
}
...
:deep(.el-table) {
    ...
    overflow: hidden;
    /* el-table 内部有 --el-table-border-color，由 cochain-tokens.scss 的
     --el-table-border-color: var(--co-divider) 统一覆盖，此处无需重复 */
}

/* 底部边框细线 */
:deep(.el-table td.el-table__cell),
:deep(.el-table th.el-table__cell) {
    border-bottom-color: var(--co-divider);
}
...
:deep(.el-table__row:hover .el-button--primary.is-link:hover) {
    color: var(--co-primary-hover) !important;
...
.pagination-container {
    ...
    margin-top: var(--co-space-12);
    padding: var(--co-space-8) 0 var(--co-space-4);
    border-top: 1px solid var(--co-divider);
}

/* 分页激活页码 — Token 主色
   cochain-tokens.scss 已覆盖 --el-pagination-hover-color，
...
/* ── 列设置抽屉内容 ── */
.column-setting {
    padding: var(--co-space-4) var(--co-space-20);
}

.column-item {
    padding: var(--co-space-12) 0;
    border-bottom: 1px solid var(--co-divider);
    transition: background var(--co-transition-fast);
}
...
.drawer-footer {
    ...
    justify-content: flex-end;
    gap: var(--co-space-8);
    padding: var(--co-space-12) var(--co-space-20);
    border-top: 1px solid var(--co-divider);
}
