    ...
    <div class="rbac-admin-page">
        ...
        >
            <template #toolbar-left="{ selection }">
                <el-button v-if="canCreate" type="primary" :icon="Plus" @click="openCreate"> 新增管理员 </el-button>
                <el-button v-if="selection.length > 0 && canDelete" type="danger" :icon="Delete" @click="handleBatchDelete(selection)">
            ...
            <template #groupNames="{ row }">
                <div class="tag-list">
                    <el-tag v-for="name in (row.groupNames || []).slice(0, 3)" :key="name" size="small" type="info" class="group-tag">{{
                        name
                    }}</el-tag>
                    <el-tag v-if="(row.groupNames || []).length > 3" size="small" type="info"> +{{ row.groupNames.length - 3 }} </el-tag>
            ...
            <template #isSuper="{ row }">
                <el-tooltip content="超管权限在「超管管理」页面调整" placement="top" :disabled="!row.isSuper">
                    <el-tag v-if="row.isSuper" size="small" class="super-tag"> <i class="fa fa-star" /> 超管 </el-tag>
            ...
            <template #operation="{ row }">
                ...
                <el-button v-if="canDelete && row.userid !== currentUserid" type="danger" link size="small" :icon="Delete" @click="handleDelete(row)"
                    >删除</el-button
                >
            </template>
        </Commontable>
        ...
        <!-- 新增/编辑抽屉 -->
        <AdminFormDrawer v-model="formDrawerVisible" :mode="formMode" :model="currentRow" @submit="handleFormSubmit" />
    </div>
</template>
...
import { getAdminList, deleteAdmin, type AdminItem } from '/@/api/backend/rbac'
import { useAdminInfo } from '/@/stores/adminInfo'

defineOptions({ name: 'auth/admin' })

const adminInfo = useAdminInfo()
const currentUserid = computed(() => adminInfo.userid ?? '')
const canCreate = computed(() => adminInfo.super === true)
const canEdit = computed(() => adminInfo.super === true)
const canDelete = computed(() => adminInfo.super === true)
...
function handleSelectionChange(rows: AdminItem[]) {
...
async function handleDelete(row: AdminItem) {
    try {
        await ElMessageBox.confirm(`确定删除管理员「${row.username}」（${row.userid}）？`, '删除确认', {
...
async function handleBatchDelete(rows: AdminItem[]) {
    try {
        await ElMessageBox.confirm(`确定批量删除选中的 ${rows.length} 位管理员？`, '批量删除确认', {
            confirmButtonText: '确定删除',
            cancelButtonText: '取消',
            type: 'warning',
        })
