<template>
    ...
    >
        <div class="drawer-body">
            <el-form ref="formRef" :model="formData" :rules="formRules" label-position="top" class="rule-form" @submit.prevent>
                <!-- ── 基础字段 ──────────────────────────────────────── -->
                <div class="form-section">
                    ...
                    <el-form-item label="规则类型" prop="type">
                        <el-radio-group v-model="formData.type" @change="handleTypeChange">
                            <el-radio-button value="menu_dir"> <i class="fa fa-folder-o" /> 目录 </el-radio-button>
                            <el-radio-button value="menu"> <i class="fa fa-file-o" /> 菜单 </el-radio-button>
                            <el-radio-button value="button"> <i class="fa fa-hand-pointer-o" /> 按钮 </el-radio-button>
                        </el-radio-group>
                    </el-form-item>

                    <!-- 显示标题 -->
                    <el-form-item label="显示标题" prop="title">
                        <el-input v-model="formData.title" placeholder="在菜单和管理界面中显示的名称" clearable />
                    </el-form-item>
                    ...
                    <el-form-item label="权限码 (permissionCode)" prop="permissionCode">
                        <div class="permission-input-wrapper">
                            <el-input v-model="formData.permissionCode" placeholder="例如：menu:system.user" clearable />
                            <el-button size="small" class="auto-gen-btn" @click="autoGenPermissionCode" :disabled="!formData.ruleCode">
                                自动生成
                            </el-button>
                        </div>
                        <div class="field-hint warning">⚠ API 映射和权限组授权依赖此字段，修改会影响已有授权关系</div>
                    </el-form-item>

                    <!-- 父规则（button 类型必填） -->
                    <el-form-item label="父规则" prop="parentRuleCode" :required="formData.type === 'button'">
                        <el-tree-select
                        ...
                        />
                        <div class="field-hint" v-if="formData.type === 'button'">按钮规则必须挂载到菜单或目录下</div>
                    </el-form-item>
                </div>
                ...
                <div class="form-section" v-if="formData.type !== 'button'">
                    ...
                    <el-form-item label="菜单类型 (menuType)" v-if="formData.type === 'menu'">
                        <el-select v-model="formData.menuType" placeholder="请选择菜单类型" clearable style="width: 100%">
                    ...
                    <!-- 外链 URL -->
                    <el-form-item label="外链 / Iframe URL" v-if="formData.type === 'menu' && ['link', 'iframe'].includes(formData.menuType)">
                        <el-input v-model="formData.url" placeholder="https://example.com" clearable />
                    </el-form-item>

                    <!-- 组件路径（tab 时显示） -->
                    <el-form-item label="组件路径 (component)" v-if="formData.type === 'menu'">
                        <el-input v-model="formData.component" placeholder="/src/views/system/user/index.vue" clearable />
                    </el-form-item>

                    <!-- Keep-alive -->
                    <el-form-item label="路由缓存 (keepalive)" v-if="formData.type === 'menu'">
                        <el-switch v-model="formData.keepalive" :active-color="'#0066cc'" active-text="开启" inactive-text="关闭" />
                ...
                <div class="form-section">
                    ...
                    <el-form-item label="排序权重 (weigh)">
                        <el-input-number v-model="formData.weigh" :min="0" :max="9999" :step="10" controls-position="right" style="width: 160px" />
...
import { createRule, updateRule, type RuleTreeNode, type RuleCreateForm, type RuleUpdateForm, type RecordStatus } from '/@/api/backend/rbac'
...
function filterButtonNodes(nodes: RuleTreeNode[]): RuleTreeNode[] {
    return nodes.filter((n) => n.type !== 'button').map((n) => ({ ...n, children: filterButtonNodes(n.children ?? []) }))
}
...
const formRules = computed(() => ({
    ...
    parentRuleCode: formData.type === 'button' ? [{ required: true, message: '按钮规则必须选择父规则', trigger: 'change' }] : [],
}))
...
function findParentRuleCode(targetCode: string, nodes: RuleTreeNode[], parentCode = ''): string | null {
...
.permission-input-wrapper .el-input {
    flex: 1;
}
.auto-gen-btn {
    flex-shrink: 0;
