<template>
    <el-drawer v-model="visible" :title="mode === 'create' ? '新增权限组' : `编辑权限组：${model?.groupName ?? ''}`" size="620px" destroy-on-close>
        <el-form ref="formRef" :model="formData" :rules="formRules" label-position="top" class="group-form">
            <!-- groupCode -->
            ...
            <el-form-item label="额外 API 端点权限">
                ...
                <div class="extra-info">
                    ...
                    <span> 追加直接来自 API 映射的权限码，不依赖规则树推导。 最终授权 = 规则推导值 ∪ 额外权限码。 </span>
                </div>
                ...
                <div class="api-panel">
                    ...
                    <div class="api-panel-toolbar">
                        <el-input v-model="apiSearch" size="small" placeholder="搜索权限码 / 标题" clearable :prefix-icon="Search" style="flex: 1" />
                        <el-select v-model="apiActionFilter" size="small" placeholder="全部 Action" clearable style="width: 120px">
...
function filterTree(nodes: RuleTreeNode[], kw: string): RuleTreeNode[] {
    return nodes.reduce<RuleTreeNode[]>((acc, node) => {
        ...
        if (node.title.includes(kw) || node.ruleCode.includes(kw) || children.length) acc.push({ ...node, children })
        return acc
    }, [])
}
...
function getCheckedRuleCodes(): string[] {
