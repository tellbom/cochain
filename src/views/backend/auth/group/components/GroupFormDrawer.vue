...
async function loadParentOptions() {
  try {
    ...
    parentGroupOptions.value = normalizeGroupOptions((result as any).options ?? [])
  } catch {}
}

function normalizeGroupOptions(options: any[]): { label: string; value: string }[] {
  return options
    .map((option) => ({
      value: String(option.value ?? option.groupCode ?? option.id ?? '').trim(),
      label: String(option.label ?? option.groupName ?? option.name ?? '').replace(/^[\s└─├│]+/, '').trim(),
    }))
    .filter((option) => option.value && option.label && option.value !== props.model?.groupCode)
}
...
function getRuleCodesFromPermissions(permissionCodes: string[] = []) {
  ...
  return flatRuleNodes(props.ruleTree)
    .filter((node) => node.permissionCode && permissionSet.has(node.permissionCode))
    .map((node) => node.ruleCode)
}
function flatRuleNodes(nodes: RuleTreeNode[]): RuleTreeNode[] {
  return nodes.flatMap((node) => [node, ...flatRuleNodes(node.children ?? [])])
}
  ...
  try {
    if (props.mode === 'create') {
      ...
      await updateGroup(formData.groupCode, {
        ...
        status:    formData.status,
        ruleCodes: getRuleCodesFromPermissions(props.model?.permissionCodes ?? []),
