    ...
    >
        <div class="drawer-body" v-loading="loading">
                ...
                <div class="section-header">
                    ...
                    <div class="section-actions">
                        ...
                        <span class="check-count"
                            >已选 <strong>{{ checkedCount }}</strong> 项</span
                        >
                    </div>
                    <el-input v-model="treeSearch" size="small" placeholder="搜索规则" clearable style="width: 150px" :prefix-icon="Search" />
                </div>

                <div class="info-tip">
                    <i class="fa fa-info-circle" />
                    父子节点互不级联。提交时仅包含当前手动勾选的 <code>ruleCode</code> 列表， 服务端推导 <code>permissionCode</code>。
                </div>
            ...
            <div class="section">
                ...
                <div class="info-tip">
                    <i class="fa fa-info-circle" />
                    追加直接来自 API 映射表的权限码，不依赖规则树推导。 最终授权 = 规则树推导值 ∪ 额外权限码。
                </div>

                <!-- 已选额外权限码 Tags -->
                <div class="extra-tags" v-if="extraPermCodes.length">
                    <el-tag v-for="code in extraPermCodes" :key="code" closable size="small" class="extra-tag" @close="removeExtraCode(code)">{{
                        code
                    }}</el-tag>
                </div>
                <div class="extra-empty" v-else>暂无额外权限码</div>

                <!-- API 映射选择器 -->
                <div class="api-picker">
                    <div class="api-picker-toolbar">
                        <el-input v-model="apiSearch" size="small" placeholder="搜索权限码" clearable :prefix-icon="Search" style="flex: 1" />
                        <el-select v-model="apiActionFilter" size="small" placeholder="Action" clearable style="width: 110px">
                    ...
                    <div class="api-list" v-loading="apiMapLoading">
                        ...
                        >
                            <el-tag size="small" :type="getActionTagType(item.action)" class="action-tag">{{ item.action }}</el-tag>
                            <code class="api-perm-code">{{ item.permissionCode }}</code>
                            <span class="api-title">{{ item.title }}</span>
                            <i class="fa fa-check-circle check-indicator" v-if="extraPermCodes.includes(item.permissionCode)" />
                        </div>
                        <div class="api-empty" v-if="!filteredApiOptions.length && !apiMapLoading">无匹配结果</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <template #footer>
            <div class="drawer-footer">
                ...
                <div class="footer-actions">
                    ...
                    <el-button type="primary" :loading="submitting" @click="handleSubmit"> 保存授权 </el-button>
                </div>
            </div>
        </template>
    </el-drawer>
...
import { updateGroupRules, getApiMapList, type GroupItem, type RuleTreeNode, type ApiMapViewItem } from '/@/api/backend/rbac'
...
const filteredApiOptions = computed(() => {
    ...
    if (apiSearch.value) {
        ...
        list = list.filter((i) => i.permissionCode.toLowerCase().includes(kw) || (i.title ?? '').toLowerCase().includes(kw))
    }
    return list
})
...
function collectRuleCodesByPermCodes(nodes: RuleTreeNode[], targetSet: Set<string>, ruleCodes: string[], matchedPermissionCodes: Set<string>) {
...
async function handleBeforeClose(done: () => void) {
    if (isDirty.value) {
        try {
            await ElMessageBox.confirm('有未保存的规则变更，确定关闭？', '提示', {
