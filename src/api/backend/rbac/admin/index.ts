 ...
 * GET /api/admin/list
 * 分页查询管理员列表。
 * 响应项包含 isSuper（当前 X-Project 超管标识）和 superProjects（超管 project 列表）。
 */
export async function getAdminList(query: AdminListQuery = {}): Promise<PagedData<AdminItem>> {
 ...
 * 新增管理员账号，可同时加入权限组。
 */
export async function createAdmin(form: AdminCreateForm): Promise<{ userid: string }> {
 ...
 * 完整编辑管理员。null 字段不修改。
 * groupArr 为目标权限组全量列表，服务端自动 diff。
 */
export async function updateAdmin(userid: string, form: AdminUpdateForm): Promise<void> {
 ...
 * 快速启用或禁用管理员。
 */
export async function updateAdminStatus(userid: string, form: AdminStatusForm): Promise<void> {
    return rbacClient.put<any, void>(`/api/admin/${encodeURIComponent(userid)}/status`, form)
}
 ...
 * 快速更新管理员显示名称。
 */
export async function updateAdminUsername(userid: string, form: AdminUsernameForm): Promise<void> {
    return rbacClient.put<any, void>(`/api/admin/${encodeURIComponent(userid)}/username`, form)
}
