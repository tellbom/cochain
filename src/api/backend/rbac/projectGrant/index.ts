 ...
 */
export async function grantProjectAccess(form: ProjectGrantForm): Promise<void> {
...
export async function revokeProjectAccess(userid: string): Promise<void> {
    return rbacClient.delete<any, void>(`/api/project-grant/${encodeURIComponent(userid)}`)
}
 ...
 * ⚠️ 超管权限变更，调用前必须二次确认。
 * 用户必须已有当前 Project 授权，否则请先调用 grantProjectAccess。
 */
export async function toggleProjectSuper(userid: string, form: ProjectGrantSuperForm): Promise<void> {
    return rbacClient.put<any, void>(`/api/project-grant/${encodeURIComponent(userid)}/super`, form)
}
