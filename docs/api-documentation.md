...
> 更新日期：2026-08-18（已对照实际 Controller、`Result`、`PageData`和当前安全配置校验）
> 基础路径：`http://{host}:8080`
> 鉴权方式：Keycloak `master` realm JWT（前端Client ID为`cochain-web`，请求头`Authorization: Bearer <token>`）。后端校验签名、有效期和issuer，不校验audience。
> 响应格式：统一包装 `Result<T>`，见 §1
> 从上传文件开始的前端联调顺序及TypeScript示例见：`前端交接-上传分包调用流程.md`
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `success` | boolean | `true`表示业务调用成功 |
| `message` | string | 成功时可能为空；错误或推荐不足时包含提示信息 |
| `code` | number | 当前成功码固定为`200`，不是`0` |
| `data` | T | 实际业务数据，部分接口可能为`null` |
| `timestamp` | number | 响应生成时间，Unix毫秒时间戳 |

前端应以`success === true && code === 200`判断成功。不要读取不存在的`msg`字段。

下文业务响应示例为突出`data`可能省略`success`或`timestamp`，实际普通JSON响应始终遵循上述完整包装结构。
...
`PageData<T>`实际只返回`list`和`total`，不会回显`pageNo`、`pageSize`；页码和每页数量由前端请求状态自行保存。
...
- 业务日期时间统一返回字符串`yyyy-MM-dd HH:mm:ss`，时区为`Asia/Shanghai`，例如`2026-08-18 16:48:19`。
- `Result.timestamp`是Unix毫秒时间戳，和业务日期时间字符串不是同一种类型。
- 时间范围查询参数使用同一格式，数组参数通过重复参数名传递，例如：`createTime=2026-08-18 00:00:00&createTime=2026-08-18 23:59:59`。
- 数据库所有17张业务表的`CREATE_TIME`、`UPDATE_TIME`已统一为`TIMESTAMP(0)`，精确到秒。
- 当前多数业务VO不直接暴露`creator/createTime/updater/updateTime`，前端只绑定接口实际返回字段，不要自行假设审计字段一定存在。
- 上传接口中的`operator`是批次业务展示字段；数据库`CREATOR`来自JWT中的工号，优先读取`employee_no`，默认可从`preferred_username`取得。

### 1.5 权限前缀（以实际代码 `@PreAuthorize` 注解为准）
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `file` | File | 是 | 分包 Excel 文件（36 列模板，尺寸使用单列“下料尺寸”） |
...
`recommendSource` 枚举：`HISTORY`（历史供应商）/ `QUALITY_ROUND`（优质轮流）/ `NORMAL_ROUND`（普通轮流）/ `CATEGORY_CAPABILITY`（普通品类按加工能力补充）/ `ALL_CATEGORY`（特殊品类全部）

铝、钛、钢普通品类的目标数量大于可加工供应商总数时，`data`保留当前全部可加工供应商，响应`message`返回逐工作包的目标数、实际数和缺口；工作包状态为`PARTIAL`（0家时为`FAILED`），批次状态为`PARTIAL`。前端应展示该`message`。
...
> 响应为 Excel 文件流（Controller声明`application/vnd.ms-excel`），不是`Result<T>`。前端必须使用`responseType: 'blob'`，具体实现见`前端交接-上传分包调用流程.md` §9.3。导出时回写推荐供应商1-7到`BATCH_PART`并将批次状态置为`COMPLETED`。
...
| `batchStatus` | String | `DRAFT` / `DATA_READY` / `PACKAGED` / `RECOMMENDED` / `PARTIAL` / `COMPLETED` |
...
| `supplierCountNeeded` | Integer | 供应商需求数 |
| `recommendCount` | Integer | 铝/钛/钢目标推荐数（=需求数+4）；非铝/钛/钢特殊品类忽略 |
...
| `comprehensiveScore` | BigDecimal | 综合得分 = 半年平均 × 0.6 + 当月原始分 × 0.4；无历史数据（首次上传/半年度首月）时回退为当月原始分 |
...
响应 `Result<RankingGenerateResultVO>`（含按品类诊断，便于定位"生成为空"的根因）：

```json
{
  "code": 200,
  "data": {
    "message": "绩效排名生成完成: 2026年7月, 共3个品类生成快照, 2个品类被跳过",
    "totalCategoryCount": 5,
    "generatedCategoryCount": 3,
    "skippedCategoryCount": 2,
    "cursorResetCount": 0,
    "categories": [
      { "categoryId": "CAT-01", "supplierCount": 2, "qualityCount": 1, "skipped": false, "skipReason": null },
      { "categoryId": "CAT-04", "supplierCount": 0, "qualityCount": 0, "skipped": true, "skipReason": "全部供应商本月均无绩效记录,无末位分可参照,本次不参与排名" }
    ]
  }
}
```

> 常见"生成为空"原因（通过 `categories[].skipped`/`skipReason` 可直接定位）：
> 1. `totalCategoryCount = 0`：`SUPPLIER_CATEGORY` 关联表为空，需先导入供应商-品类关联；
> 2. 品类 `skipped = true` 且原因含"无绩效"：该品类关联供应商本月未上传绩效（`SUPPLIER_PERFORMANCE` 无该年月数据），需先上传绩效再生成。
...
```
POST /api/supplier/category/import (multipart/form-data)
参数: file (MultipartFile) - Excel文件
权限: supplier:category:import
```

Excel 两列（含表头）：`供应商名称`、`三级品类名称`（均为中文名称，系统按名称精确匹配自动映射 ID；匹配失败的行不阻断其余行，统一在响应中返回）。

成功响应 `Result<CategoryImportResultVO>`：
...
```json
{
  ...
  "data": {
    ...
    "errorCount": 3,
    "errors": [
      { "rowNo": 25, "supplierName": "不存在供应商测试", "categoryName": "小型标准件", "message": "供应商[不存在供应商测试]不存在" },
      { "rowNo": 26, "supplierName": "航宇精工制造有限公司", "categoryName": "不存在的三级品类测试", "message": "三级品类[不存在的三级品类测试]不存在" },
      { "rowNo": 27, "supplierName": "不存在供应商测试", "categoryName": "不存在的三级品类测试", "message": "供应商[不存在供应商测试]不存在;三级品类[不存在的三级品类测试]不存在" }
    ]
  }
}
```

`rowNo` 为 Excel 工作表真实行号（从 1 开始、含表头）；解析/格式校验失败行无名称信息时 `supplierName`/`categoryName` 为 `null`。
...
> 当前业务口径以材料类型为准：除铝/钛/钢外全部为特殊品类，直接推荐该品类全部启用加工供应商且不校验数量。此配置接口仅保留`specialType`展示兼容，`recommendRule/ignoreQuality`不再决定推荐分支。
...
| `recommendRule` | String | 兼容字段；当前推荐分支不再读取 |
...
| 34 | 备注 | 否 | |
| 35 | 下料尺寸 | 否 | 格式为`长*宽`，例如`1200*600`；导入后解析为内部长度、宽度 |
...
| 枚举 | 值 |
|------|----|
| 批次状态 | `DRAFT` / `DATA_READY` / `PACKAGED` / `RECOMMENDED` / `PARTIAL` / `COMPLETED` |
| 零件类型 | `小型` / `中型` / `大型` / `超大型` / `其他` |
| 推荐来源 | `HISTORY` / `QUALITY_ROUND` / `NORMAL_ROUND` / `ALL_CATEGORY` |
| 质量等级 | `优质` / `普通` |
| 特殊品类 | 材料类型非铝/钛/钢；`specialType`优先取兼容配置，否则为实际材料类型 |
| 推荐规则 | 特殊品类固定为全部启用加工供应商，不校验数量 |
